...
export function BootScreen({ onReady }: BootScreenProps) {
  ...
  // Check Tauri backend status
  const checkTauriStatus = useCallback(async () => {
    try {
      // @tauri-apps/api might not be available in browser dev mode
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      // Wait a moment for the backend process to start
      await new Promise((r) => setTimeout(r, 2000));

      while (active) {
        // 1. Try Tauri command first
        const tauriStatus = await checkTauriStatus();
        if (tauriStatus) {
          if (!active) break;
          setBackendStatus(tauriStatus);

          if (tauriStatus === "ready") {
            setStatusMsg("✅ 后端服务就绪");
            setTimeout(() => active && onReady(), 600);
            return;
          }
          if (tauriStatus === "jar_not_found") {
            setStatusMsg("❌ 未找到后端服务文件 (JAR)");
            return;
          }
          if (tauriStatus === "failed") {
            setStatusMsg("❌ 后端服务启动失败，请查看日志");
            return;
          }
          if (tauriStatus.startsWith("spawn_error")) {
            setStatusMsg(`❌ ${tauriStatus}`);
            return;
          }
          // "starting" → keep polling
        }

        // 2. Also try HTTP health check (works in dev mode too)
        try {
  ...
  return (
    ...
    >
      ...
      <div
        style={{ position: "absolute", top: 40, left: 60, opacity: 0.08 }}
      >
      ...
      <div
        style={{ position: "absolute", bottom: 60, right: 80, opacity: 0.06 }}
      >
      ...
      <div style={{ textAlign: "center", position: "relative", zIndex: 1 }}>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          >
            ...
            <g opacity="0.25">
              <path
                d="M 40 380 Q 100 250 160 300 Q 200 200 260 280 Q 300 180 360 260 Q 400 200 470 350"
                fill="#2d2d44"
              />
            </g>
            <g opacity="0.45">
              <path
                d="M 30 400 Q 80 280 140 330 Q 180 240 230 310 Q 270 220 320 290 Q 370 230 420 320 Q 450 270 482 380"
                fill="#1a1a2e"
              />
            </g>
            <g opacity="0.7">
              <path
                d="M 50 420 Q 120 310 180 360 Q 230 280 280 340 Q 340 260 400 330 Q 440 290 480 400 L 482 420 Z"
                fill="#1a1a2e"
              />
            </g>
