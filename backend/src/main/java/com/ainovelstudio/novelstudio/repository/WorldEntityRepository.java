...
public interface WorldEntityRepository {
    ...
    @Results({
        @Result(property = "aliases", column = "aliases", typeHandler = com.ainovelstudio.novelstudio.config.JsonStringListTypeHandler.class),
    ...
    @Insert("INSERT INTO world_entities (id, novel_id, type, name, aliases, summary, detail_json, locked, tags) " +
            "VALUES (#{id}, #{novelId}, #{type}, #{name}, #{aliases, typeHandler=com.ainovelstudio.novelstudio.config.JsonStringListTypeHandler}, #{summary}, #{detailJson}, #{locked}, #{tags, typeHandler=com.ainovelstudio.novelstudio.config.JsonStringListTypeHandler})")
    int insert(WorldEntity entity);

    @Update("UPDATE world_entities SET name=#{name}, aliases=#{aliases, typeHandler=com.ainovelstudio.novelstudio.config.JsonStringListTypeHandler}, summary=#{summary}, detail_json=#{detailJson}, " +
            "locked=#{locked}, tags=#{tags, typeHandler=com.ainovelstudio.novelstudio.config.JsonStringListTypeHandler}, updated_at=datetime('now') WHERE id=#{id}")
