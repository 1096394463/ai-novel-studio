...
public interface IdeaRepository {
    ...
    @Results({
        ...
        @Result(property = "relatedEntityIds", column = "related_entity_ids", typeHandler = JsonStringListTypeHandler.class)
    })
    ...
    @Insert("INSERT INTO ideas (id, novel_id, title, content, status, tags, related_entity_ids, suggested_chapter_id) " +
            "VALUES (#{id}, #{novelId}, #{title}, #{content}, #{status}, #{tags, typeHandler=JsonStringListTypeHandler}, #{relatedEntityIds, typeHandler=JsonStringListTypeHandler}, #{suggestedChapterId})")
    int insert(Idea idea);

    @Update("UPDATE ideas SET title=#{title}, content=#{content}, status=#{status}, tags=#{tags, typeHandler=JsonStringListTypeHandler}, " +
            "related_entity_ids=#{relatedEntityIds, typeHandler=JsonStringListTypeHandler}, suggested_chapter_id=#{suggestedChapterId}, " +
